...
enum Localization {
    ...
    private static let csPairs: [(String, String)] = [
        ...
        ("used", "využito"),
        ("cores", "jader"),
        // Metric detail popovers (Battery / Memory / CPU / Network / Disk)
        ("used of", "z"),
        ...
        ("charge cycles", "cyklů nabití"),
        ("Health", "Zdraví"),
        ("Temperature", "Teplota"),
        ("Gatekeeper", "Gatekeeper"),   // macOS proper noun — kept as-is (like FileVault/SIP/XProtect)
        ("Your battery is in good condition.", "Baterie je v dobrém stavu."),
        ("Your battery has lost some capacity.", "Baterie ztratila část kapacity."),
        ("Within its normal working range.", "V normálním pracovním rozsahu."),
        ("used · load", "využito · zátěž"),
        ("App", "Appky"),
        ("Wired", "Rezervováno"),
        ...
        ("Config transfer", "Přenos konfigurace"),
        ("Save your allowlist and rules to a file to carry to another Mac or back up. Import merges — it never drops what you already have. Secrets (API keys) are never exported.", "Ulož allowlist a pravidla do souboru pro přenos na jiný Mac nebo zálohu. Import slučuje — nikdy nezahodí, co už máš. Tajemství (API klíče) se nikdy neexportují."),
        ("Export", "Exportovat"),
        ("Import", "Importovat"),
        ("Exported config to", "Konfigurace exportována do"),
        ("Couldn't export the config.", "Konfiguraci se nepodařilo exportovat."),
        ("Imported", "Importováno"),
        ("exclusions and", "vyloučení a"),
        ("rules.", "pravidel."),
        ("Couldn't import — is it a valid Kestrel config file?", "Import selhal — je to platný Kestrel config?"),
        ...
        ("Reclaim everything safe", "Uvolnit vše bezpečné"),
        ("Trash, leftover data of removed apps, developer caches and crash logs — all regeneratable — in one pass. Everything moves to the vault (undoable).", "Koš, zbytky po odebraných appkách, vývojářské cache a crash logy — vše znovu-generovatelné — jedním průchodem. Vše jde do trezoru (vratně)."),
        ("Move all to Vault", "Přesunout vše do trezoru"),
        ("Reclaim", "Uvolnit"),
        ("These regeneratable items all move to the reversible vault — undoable until you purge.", "Tyto znovu-generovatelné položky jdou do vratného trezoru — vratné do vyprázdnění."),
        ("Messages Attachments", "Přílohy ze Zpráv"),
        ("Photos/videos received in Messages", "Fotky/videa přijaté ve Zprávách"),
        ("Crash Reports", "Zprávy o pádech"),
        ("Diagnostic logs — regeneratable", "Diagnostické logy — znovu-generovatelné"),
        ("Xcode Archives", "Xcode archivy"),
        ("Old .xcarchive builds — review only", "Staré .xcarchive buildy — jen ke kontrole"),
        ("iOS backups", "iOS zálohy"),
        ("Duplicate apps", "Duplicitní aplikace"),
        ("Apps found in more than one place or version — e.g. a stale copy left in Downloads. Kestrel keeps the best one; move the extras to the vault (undoable).", "Aplikace nalezené na více místech nebo ve více verzích — např. stará kopie zapomenutá ve Stažených. Kestrel nechá tu nejlepší; přebytky přesuň do trezoru (vratně)."),
        ("Scan to find duplicate app copies.", "Skenem najdeš duplicitní kopie aplikací."),
        ("keep", "nechat"),
        ...
        ("y", "l"),
        ("Copy command", "Kopírovat příkaz"),
        ("Free up space to keep things responsive.", "Uvolni místo, ať vše svižně reaguje."),
        ("From the TCC database", "Z databáze TCC"),
        ("Measuring bundle sizes", "Měřím velikosti aplikací"),
        ("One honest pass across cleanup, protection and malware.", "Jeden poctivý průchod: úklid, ochrana a malware."),
        ("Purge old sessions in Settings to reclaim it for good.", "Vyprázdni staré relace v Nastavení a uvolni to natrvalo."),
        ...
        ("Undo last", "Vrátit poslední"),
        ("Nothing to undo — the vault is empty.", "Není co vrátit — trezor je prázdný."),
        ("Restored", "Obnoveno"),
        ("item(s) from the last cleanup.", "položek z posledního úklidu."),
        ("Couldn't undo the last cleanup (files may already be back).", "Poslední úklid nešel vrátit (soubory už možná jsou zpět)."),
        ...
        ("Docker & Homebrew", "Docker & Homebrew"),
        ("Reclaimable space in tools that manage their own storage. Advisory — copy the command to run it yourself; Kestrel won't prune these (they can't go through the vault).", "Uvolnitelné místo v nástrojích, co si spravují vlastní úložiště. Poradensky — zkopíruj příkaz a spusť sám; Kestrel je nepromazává (nejdou přes trezor)."),
        ("Check for reclaimable Docker/Homebrew space.", "Zkontroluj uvolnitelné místo v Dockeru/Homebrew."),
